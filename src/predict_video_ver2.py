...
def create_minimap(court_detector, tracker, ball_detector, fps):
    ...
    frame_num = 0
    smoothed_1, smoothed_2 = tracker.calculate_feet_positions(court_detector)
    for feet_pos_1, feet_pos_2 in zip(smoothed_1, smoothed_2):
        frame = court.copy()
        if feet_pos_1[0] is not None:
            frame = cv2.circle(frame, (int(feet_pos_1[0]), int(feet_pos_1[1])), 30, (255, 0, 255), -1)
        if feet_pos_2[0] is not None:
            frame = cv2.circle(frame, (int(feet_pos_2[0]), int(feet_pos_2[1])), 30, (255, 255, 0), -1)
